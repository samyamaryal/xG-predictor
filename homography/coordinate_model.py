...
class CoordinateModel:
    ...
    def __init__(self, keypoint_conf: float = 0.3, detector_conf: float = 0.35):
        ...
        self.detector_model = YOLO("weights/detector_large_hd.pt").to(device)
        self.keypoint_model = KeypointModel(57).to(device)
        self.keypoint_model.load_state_dict(torch.load("weights/keypoints_main.pth"))
    ...
    def get_coordinates(self, frames: np.ndarray, fps: int, num_homography: int = 1, num_keypoint_detection: int = 1, detector_interval: int = 2, verbose: bool = True, calibration: bool = False) -> dict:
        """
        ...
        try:
            ...
            if len(indices) > 0:
                ...
                for idx in indices:
                    ...
                    # Do transforms on CPU and defer moving to device until we have a stacked batch
                    batched.append(self.transforms(image=img)["image"])
                    batched_idx.append(idx)
                    if len(batched) == BATCH:
                        # Move stacked batch to device once to reduce repeated transfers
                        batch_tensor = torch.stack(batched, dim=0).to(self.keypoint_model.unnormalized_model[1].weight.data.device).float()
                        t0 = time.perf_counter()
                        kp_list = self.keypoint_model.get_keypoints(batch_tensor)
                        dt = time.perf_counter() - t0
                        profiling["keypoint_time"] += dt
                        profiling["keypoint_count"] += 1
        ...
        for i, frame in tqdm(enumerate(frames), desc="Processing Frames", total=len(frames)) if verbose else enumerate(frames):
                ...
                if len(keypoints) < 4:  # Model only detected less than 4 keypoints, combine with optical flow
                    if i == 0:
                        ...
                        for j in range(i + 1, len(frames)):
                            ...
                            if j in mem:
                                next_keypoints = mem[j]
                            else:
                                t0 = time.perf_counter()
                                next_keypoints = self.detect_keypoints(next_frame)
                                dt = time.perf_counter() - t0
                                profiling["keypoint_time"] += dt
                                profiling["keypoint_count"] += 1
                        ...
                        dt = time.perf_counter() - t0
                        profiling["optflow_time"] += dt
                        profiling["optflow_count"] += 1
            ...
            prev_gray = curr_gray
            # Run detector only on configured interval to save inference time
            if (i % max(1, detector_interval)) == 0:
                objects = self.detect_objects(frame, run_detector=True)
            else:
                # Advance tracker without running detector (use empty detections)
                objects = self.detect_objects(frame, run_detector=False)
            if i % homography_interval == 0 or compute_homography:
                ...
                profiling["homography_time"] += (time.perf_counter() - t0)
                profiling["homography_count"] += 1
        ...
        # Attach simple profiling summary to results under a reserved key
        profiling_summary = {}
        for k in ["keypoint", "detector", "optflow", "homography"]:
            total = profiling.get(f"{k}_time", 0.0)
            count = profiling.get(f"{k}_count", 0)
            profiling_summary[f"{k}_avg_ms"] = (total / count * 1000.0) if count > 0 else 0.0
            profiling_summary[f"{k}_count"] = count
        res["_Profiling"] = profiling_summary
        return res
    ...
    def calculate_optical_flow(self, frame: np.ndarray, prev_gray: np.ndarray, prev_keypoints: dict, curr_gray: np.ndarray):
        ...
        # Precompute HSV conversion once for this frame to avoid repeated cv2.cvtColor calls
        try:
        ...
        for j, (point, new_point) in enumerate(zip(prev_points, new_points)):
            ...
            if hsv_frame is None:
                ...
                curr_grid_hsv = hsv_frame[curr_y_min:curr_y_max, curr_x_min:curr_x_max]
                if curr_grid_hsv.size == 0:
                    avg_hue_curr = 0
                else:
                    avg_hue_curr = np.mean(curr_grid_hsv[:, :, 0])
            ...
            if hsv_frame is None:
                ...
                prev_grid_hsv = hsv_frame[prev_y_min:prev_y_max, prev_x_min:prev_x_max]
                if prev_grid_hsv.size == 0:
                    avg_hue_prev = 0
                else:
                    avg_hue_prev = np.mean(prev_grid_hsv[:, :, 0])
    ...
    @torch.no_grad()
    def detect_objects(self, frame: np.ndarray, run_detector: bool = True, detections: np.ndarray = None):
        """
        ...
        """
        # If requested, run the detector and prepare detection array for the tracker.
        if run_detector:
            ...
            if coords.shape[0] > 0:
                det_array = np.hstack((coords, conf.reshape(-1, 1), class_labels.reshape(-1, 1)))
            else:
                det_array = np.empty((0, 6))
        else:
            # Advance tracker without detector: use provided detections or empty array
            if detections is not None:
                det_array = detections
            else:
                det_array = np.empty((0, 6))
            # Ensure downstream variables exist (empty) so fallback logic doesn't break
            coords = np.empty((0, 4))
            conf = np.empty((0,))
        ...
        # Fallback: if tracking yields no players/goalkeepers but we ran the detector and detections exist, use raw detections
        if run_detector and (len(res["Player"]) == 0 and len(res["Goalkeeper"]) == 0) and coords.shape[0] > 0:
        ...
        # Ensure ball detection is populated either from raw detections (when run) or from tracker outputs
        # First, try to add balls from detector outputs if available
        if run_detector and coords.shape[0] > 0 and 2 in class_labels:
            ...
            # If detector not run, try to infer ball from tracker outputs
            ball_count = 0
            for track_item in tracks:
                x1, y1, x2, y2, id, curr_conf, class_idx, idx = track_item
                if int(class_idx) == 2 and float(curr_conf) >= self.detector_conf:
                    box = np.array([x1, y1, x2, y2], dtype=int)
                    if "Ball" not in res:
                        res["Ball"] = {}
                    res["Ball"][ball_count] = {"BBox": box, "Confidence": float(curr_conf), "Bottom_center": [int((box[0] + box[2]) / 2), box[3]]}
                    ball_count += 1
        return res

...
class KeypointModel(nn.Module):
    ...
    def get_keypoints(self, x: torch.Tensor):
        ...
        """
        # Forward pass returns (N, C, H, W) -- run without gradient tracking
        with torch.no_grad():
            batch_heatmaps = self.forward(x)  # Shape: (N, n_heatmaps, H, W)
        # Use vectorized torch ops to find argmax per heatmap on the current device
        N, C, H, W = batch_heatmaps.shape
        # Flatten spatial dims and get max values and indices
        flat = batch_heatmaps.view(N, C, -1)
        vals, idx = flat.max(dim=2)  # vals: (N, C), idx: (N, C)
        ...
        # Move small result tensors to CPU once and build same return format
        vals_cpu = vals.detach().cpu().numpy()
        x_n_cpu = x_n.detach().cpu().numpy()
        y_n_cpu = y_n.detach().cpu().numpy()

        batch_coords = []
        for n in range(N):
            coords = []
            for i in range(C):
                score = float(vals_cpu[n, i])
                if score > 0.01:
                    coords.append((i, float(x_n_cpu[n, i]), float(y_n_cpu[n, i]), score))
